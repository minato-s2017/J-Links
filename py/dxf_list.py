...
# 2列目以降は共通。
# 値＝箱の左端からの距離。左揃え列はテキスト左端、中央揃え列(spl_l/N/M/E1/P1)はテキスト中心の位置。
BOX_SHIFT_H  = [513, 197, 212, 209, 266.17, 244.76, 149.2, 149.2, 197.40, 197.40, 432, 110]  # 頭マーク "H"
BOX_SHIFT_SH = [434, 197, 212, 209, 266.17, 244.76, 149.2, 149.2, 197.40, 197.40, 432, 110]  # 頭マーク "SH" / "BH"
...
EMPTY_BAR_CHAR = "ー"   # 置換文字: カタカナ長音記号「ー」
EMPTY_BAR_SHIFT = 361.6     # 「ー」の X 位置（箱の左端からの距離）

# H.T.BOLT（フランジ/ウェブ）でボルトが無い場合の "-"（_bolt_text 由来）は CAD に描かない。
#   ※実データでは H.T.BOLT が空になることは無い前提。プレースホルダの棒を出さないための削除（ユーザー指定）。
HTBOLT_KEYS = ("f_bolt", "w_bolt")   # H.T.BOLT 列
HTBOLT_EMPTY = "-"                   # ボルト無しプレースホルダ（描画しない）
